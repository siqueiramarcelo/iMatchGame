...
- (void)setCards {
    ...
    if (IS_IPHONE_4) {
        ...
        gridCollums = 6;
        gridRows = 10;
        buttonWidth = cardDimension - 8;
        buttonHeight = cardDimension - 8;
        startX = 14.0;
        startY = (-cardDimension / 2);
        gridGapX = 18;
        gridGapY = 5.5;
        
        //if (IS_SYSTEM_6) [self adjustButtons];
...
- (void)adjustButtons {
    ...
    float newHeight = IS_SYSTEM_6 ? 415 : IS_IPHONE_5 ? 488 : 402;
    CGRect frameNow;
    
    frameNow = CGRectMake(self.restartButton.frame.origin.x, newHeight, self.restartButton.frame.size.width, self.restartButton.frame.size.height);
    self.restartButton.frame = frameNow;
    
    frameNow = CGRectMake(self.pointsDisplay.frame.origin.x, newHeight, self.pointsDisplay.frame.size.width, self.pointsDisplay.frame.size.height);
    self.pointsDisplay.frame = frameNow;
    
    frameNow = CGRectMake(self.timeDisplay.frame.origin.x, newHeight, self.timeDisplay.frame.size.width, self.timeDisplay.frame.size.height);
    self.timeDisplay.frame = frameNow;
    
    frameNow = CGRectMake(self.gameCenterButton.frame.origin.x, newHeight, self.gameCenterButton.frame.size.width, self.gameCenterButton.frame.size.height);
